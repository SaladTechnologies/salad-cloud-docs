...
Salad AI Gateway offers two payment options:

- **Monthly subscription** - a flat-rate plan with a limit of two concurrent requests.
  [Contact Support](/support/contact) to request this option.
- **Pay per token** - usage-based billing at the
  [published input and output token rates](/ai-gateway/reference/pricing), without the monthly plan's concurrency limit.
  Follow [Pay-Per-Token Onboarding](/ai-gateway/tutorials/pay-per-token-onboarding) to get started.
...
curl https://ai.salad.cloud/v1/chat/completions \
  ...
  -H "Authorization: Bearer <your-ai-gateway-api-key>" \
  -d '{
...
authentication instead of the `Salad-Api-Key` custom header**. Set your organization-specific AI Gateway key as the API
key in whichever field the tool provides (the `Authorization: Bearer` header), and point the base URL to
`https://ai.salad.cloud/v1`.
...
export OPENAI_API_KEY=your-ai-gateway-api-key
```
