...
We have integration guides for many popular agentic tools - Cline, Aider, OpenCode, OpenClaw, Kilo Code, Goose, Hermes
Agent, and more. All of those guides apply here with one difference: **use standard authentication instead of the
`Salad-Api-Key` custom header**. Set your Salad API key as the API key in whichever field the tool provides (the
`Authorization: Bearer` header), and point the base URL to `https://ai.salad.cloud:40404/v1`.
