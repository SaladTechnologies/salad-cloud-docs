...
Credits are checked when each request is admitted. If credits run out during generation, requests already admitted are
allowed to complete and are billed; new requests return `402 credits_exhausted` until the balance is positive.
...
**Manage** or return to that organization's **AI Gateway** page. Follow
[Manage Your AI Gateway API Key](/ai-gateway/how-to-guides/manage-api-key) for expiration delays, deletion, and what
happens when organization membership changes.
