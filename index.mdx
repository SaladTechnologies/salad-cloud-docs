...
<Visibility for="agents">
  > For autonomous Container Engine, AI Gateway, Transcription API, or Transcription Lite work, begin with the [Agent
  > Operations Overview](/agents/overview). Use live API responses for dynamic state and the selected product's current
  > OpenAPI specification where provided; examples on human pages are not current account facts.
</Visibility>
...
Using Claude Code, Codex, Cursor, or your own agent to deploy and operate SaladCloud, or to write code that runs here?
[Build with AI Agents](/general/tutorials/build-with-ai-agents) covers the SaladCloud skills
(`npx skills add https://docs.salad.com`), the documentation MCP server, API keys for agents, and the
[Agent Operations](/agents/overview) runbooks.
...
<CardGroup cols={1}>
  ...
  <Card title="Salad AI Gateway" icon="microchip-ai" href="/ai-gateway/explanation/overview">
    ...
    cold starts. Just point your app or tool at `https://ai.salad.cloud/v1` and start generating. Available in beta with
