...
1. **Request Zap**: This Zap will trigger when a Google Form (or any other app of your choice) is submitted, sending a transcription request to the Salad service.
2. **Response Zap**: This Zap will catch the response from the Salad service and send the transcription result via email (use any app you like).
...
2. **Create a New Zap**: On the home page, click the "Create" button and select "Zaps".
   <img src="/inference-endpoints/images/zappier1.png" />
...
12. **Configure the Action:**
    1. Fill in the required fields:
       ...
       2. **URL**: Enter your Salad API endpoint URL. For example: https://api.salad.com/api/public/organizations/{your_organization_name}/inference-endpoints/transcribe/jobs. Replace {your_organization_name} with the name of your organization in Salad.
...
- _For "url": Click inside the quotation marks and select "File link" from the dropdown to insert the data from the Google Form._
- _For "webhook": Paste the webhook URL you copied from the Response Zap._

1. **Headers:**
   1. Under “Headers" enter the following key-value pairs:

      **Key**: Salad-Api-Key, Value: (your Salad API key)
...
1. **Submit the Google Form**: When you submit the form with the audio file link, it triggers the Request Zap, which sends the transcription request to the Salad service.
2. **Transcription Process**: The Salad service processes the transcription request.
3. **Receive Transcription Result**: The Response Zap catches the result from the Salad service.
4. **Send Email with Result**: The Response Zap then sends an email with the transcription result to the specified recipient.

Congratulations! You have successfully created an automated audio transcription workflow using Google Forms, Zapier, and Salad. Note that transcripts will be available for up to 24.
