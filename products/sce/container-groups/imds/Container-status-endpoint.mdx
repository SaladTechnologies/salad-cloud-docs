...
Now, if we run this on a container that has passed its [startup probe](/container-engine/health-probes/startup-probes),
but not completed its [readiness probe](/container-engine/health-probes/readiness-probes), we'll see this:
