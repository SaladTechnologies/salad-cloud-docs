...
As an example, running `docker run --entrypoint /bin/test mycontainer:latest arg1 arg2 arg3` would correspond to the
...
`docker run --entrypoint /bin/bash mydocker/mycontainer:latest -c "python server.py"`, you would enter `/bin/bash` as
