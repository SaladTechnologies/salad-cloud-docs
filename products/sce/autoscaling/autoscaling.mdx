...
To use the autoscaling feature, you need to integrate the Salad queue into your container groups. The system will
monitor the queue length and adjust the number of container replicas accordingly.

## Prerequisites

Before you can enable autoscaling, you must configure the Salad Queue:

1. Ensure that the [**Salad Queue**](/products/sce/job-queues/job-queues) is enabled for your container group.
2. Verify that your application can handle scaling with multiple container instances.
3. Review the [Autoscaling Settings](/products/sce/autoscaling/settings) to understand how to fine-tune autoscaling for
   your workloads.
...
any resources when there is no workload. However, this may result in longer startup times when new jobs arrive, as the
system needs to spin up instances again.
...
Ready to configure autoscaling? Learn more about the available [Settings](/products/sce/autoscaling/settings) or
[Set Up Autoscaling](/products/sce/autoscaling/enable-autoscaling) in SaladCloud.
