...
- [Salad Gateway Service (SGS)](/gateway-service/introduction): a proxy service that routes requests from VPN operators
  or other customers who need distributed residential IP infrastructure to SaladCloud nodes.
- [Salad Transcription API](/products/transcription/transcription): the lowest-priced transcription service in the
  market with unparalleled accuracy.
