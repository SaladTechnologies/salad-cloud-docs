...
container instances running on the node. When this happens we will automatically reallocate these instances to a new
...
container group instances will not be interrupted. However, high priority instances will still be reallocated if your
container exits, you request a reallocation, or the node powering the instance disconnects. Container groups without a
GPU always run at the Batch priority level, and can be interrupted by any container group with a GPU.
