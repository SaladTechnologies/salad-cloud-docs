...
**SCEs are not virtual machines with attached volumes; instead, both the image and any runtime data are removed from the
nodes when the application is stopped. Additionally, external SSH access is not supported.**
