...
SCEs are **not virtual machines or physical machines** with attached volumes; instead, both the image and any runtime
data are removed from the nodes when the application is stopped. Additionally, **external SSH access is not supported**
...
- [Utilize a load balance](/products/sce/gateway/networking) for real-time applications (Allowing up to 100 seconds of
