...
SCE instances are **not virtual machines or physical machines** with attached volumes; instead, both the image and any runtime
data are removed from the physical nodes when the application is stopped. Additionally, **external SSH access is not supported**
and **running docker run commands inside SCE instances (Docker-in-Docker) is not permitted.**
...
- [Utilize a load balance](/products/sce/networking) for real-time applications (Allowing up to 100 seconds of
