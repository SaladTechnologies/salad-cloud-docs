...
runtime data are removed from the physical nodes when the application is stopped. Additionally, **external SSH access is
not supported** and **running docker run commands inside SCE instances (Docker-in-Docker) is not permitted.**
