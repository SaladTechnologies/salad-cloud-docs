...
- Network performance will [vary from node to node](/products/sce/salad-container-engine#performance-variability), due
  the distributed, residential nature of the network.
...
SaladCloud Container Engine automatically reallocates your workload to another GPU (same type and class) when a resource
goes offline.
...
- [SaladCloud Gateway Service (SGS)](https://docs.salad.com/gateway-service/introduction): a proxy service that routes
  requests from VPN operators or other customers who need distributed residential IP infrastructure to SaladCloud nodes.
...
- SaladCloud bills based on resource usage at the org level. You can view your current bill at the org level.
