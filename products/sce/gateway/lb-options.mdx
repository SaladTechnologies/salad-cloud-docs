...
be changed once the container group is created. To modify the container gateway settings, you can replicate the existing
...
**When the option - “Limit each server to a single, active connection” is selected**, there are two possible cases:

**- For servers using HTTP/1.1**, the container gateway will use a single active connection per instance and only
forwards one request at a time to an instance. **New requests are waiting and queued by the container gateway until the
current one is processed and a response is returned by the instance.** This configuration ensures that an instance
receives and processes only one request at any time, with a WebSocket connection treated as a single request.

**- For servers using HTTP/2**, the container gateway will use a single active connection per instance, but it can still
send concurrent requests to an instance because of HTTP/2’s multiplexing capabilities. So, the servers need to handle
multiple requests simultaneously.

For single inference tasks, such as generating an image at a time, and using HTTP/1.1, choosing this option can
significantly simplify the implementation of the inference server by using synchronous calls. **Otherwise, this option
will reduce system throughput and should be avoided.**
...
**When a container gateway is configured to use multiple connections per instance or multiple streams with HTTP/2, this
setting can be ignored**, as all requests are sent to the instance immediately and would be queued by the inference
server.

**However, if a container gateway is configured to use a single active connection per instance and for HTTP/1.1 servers,
the setting is crucial in influencing the system behavior:**
...
To accommodate your application’s needs, you can create a new container group by replicating the existing one and
