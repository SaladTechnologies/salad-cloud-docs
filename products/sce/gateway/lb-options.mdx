...
This server should be capable of handling multiple requests simultaneously, include a local queue, use non-blocking I/O
and have a back-pressure mechanism.
...
A 100-second timeout is generally sufficient for many tasks, including image generation and classification. However,
for LLMs, the decoding process for very long conversions may take a few minutes. **If the inference server operates in
