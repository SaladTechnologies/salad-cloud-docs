...
registries supported by SaladCloud at [Container Registries](/products/sce/container-groups/registries/container-registries).
...
- By default, the Worker logging level is set to `error`. Logs will be sent to the
  [SaladCloud Container Logs](/products/sce/container-groups/container-logs) or your [external logging](/products/sce/container-groups/external-logging/external-logging)
  tools. You can adjust the logging level by adding a 'SALAD_QUEUE_WORKER_LOG_LEVEL'
  [environment variable](/products/sce/container-groups/environment-variables).
...
- If you choose not to use any Startup and/or Readiness probes, the Job Queue Worker will immediately start to look for
  ...
  need to submit it again. [Read more about health probes here. ](/products/sce/container-groups/health-probes/health-probes)
