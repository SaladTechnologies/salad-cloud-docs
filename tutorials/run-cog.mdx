...
The provided [command and its arguments](/products/sce/container-groups/specifying-a-command) override both the
...
[the HTTP/1.X probe](/products/sce/container-groups/health-probes/health-probes#http-1-x-protocol) checks the health of
an HTTP service running inside the container by sending an HTTP GET request and checking the response code.
...
Because the [command and its arguments](/products/sce/container-groups/specifying-a-command) override both the
...
[the protocol - exec](/products/sce/container-groups/health-probes/health-probes#exec-protocol), will run the given
command inside the container, if the command returns an exit code of 0 ( by using ‘echo $?’ ), the container is
...
control over how the image is built. You can refer to [this guide](/guides/computer-vision/deploy-blip-with-cog) on
writing a Dockerfile from scratch by leveraging the Pytorch image for your AI applications.
...
[this guide](/products/sce/gateway/enabling-ipv6#ipv6-with-cog-through-socat) for more information. Alternatively, you
can use the approaches in Scenario 1 or Scenario 2 to add IPv6 support later.
