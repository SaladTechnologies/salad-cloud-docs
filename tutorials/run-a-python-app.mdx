...
an unique container name. The format of the image name is `{dockerhubusername}/{imagename}:{version}`
...
Now that you have a complete (yet basic) FastAPI application running you can deploy it to Salad via the Portal or the
Public API.
...
[Deployment Guide](/products/sce/deploy-containers)
