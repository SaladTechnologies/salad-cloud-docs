...
This tutorial will guide you through the process of running a container locally and deploying a GPU-accelerated application on SaladCloud, following best practices. Before you begin, please complete [the quickstart guide](https://docs.salad.com/products/quickstart) to set up your account, organization and project on SaladCloud. This tutorial assumes you have a basic understanding of containers and images, Python, Docker commands, job queues, and load balancers.
...
[Docker Desktop with the WSL 2 backend on Windows](https://docs.docker.com/desktop/gpu/) significantly simplifies running GPU-accelerated containers, eliminating the need for complex setups or a separate Linux environment.
[Integrating Visual Studio Code with WSL](https://code.visualstudio.com/docs/remote/wsl) offers a powerful, efficient, and seamless development experience, enabling developers to leverage the strengths of both Windows and Linux environments without the overhead of managing separate setups.
...
<img src="/tutorials/images/docker_run/docker_run1.png" />


# Example Code and Image


**Let’s use an example to illustrate how this feature works and how to utilize it effectively.** 
Assume we have a container image built from [the following Dockerfile](https://github.com/SaladTechnologies/misc/blob/main/Test/Dockerfile):
...
This Dockerfile sets up a containerized environment using the PyTorch base image with GPU support. 
It installs Flask and the necessary dependencies, and copies the Python scripts into the image. 
When the container is run, it starts two Python processes:  **inference_server.py**, which runs in the background to handle inference requests, and **io_worker.py**, which manages input/output operations and invokes the inference.


The [inference_server.py](https://github.com/SaladTechnologies/misc/blob/main/Test/inference_server.py) simulates a classical API server that handles requests and runs GPU-based inference tasks, such as transcription, image or text generation, or molecular dynamics simulation, etc. 
In this example, we create a simple Flask web application using the two environment variables (‘HOST’ and ‘PORT’). When accessed at the root URL, it returns a message displaying the versions of PyTorch, CUDA, cuDNN, and the name of the first GPU available on the system. 

When 'HOST' is set to **'0.0.0.0'**, the Flask built-in server listens on all available network interfaces for IPv4 addresses, and does not handle IPv6 requests. 
In contrast, when configured to **'::'**,  the server can accept both IPv4 and IPv6 requests from all available interfaces. 
**However, some web servers or images may be set to handle only IPv6 traffic when using ‘::’ or ‘*’.**
...
The [io_worker.py](https://github.com/SaladTechnologies/misc/blob/main/Test/io_worker.py) simulates a typical workflow by continually interacting with a job queue (AWS SQS or others) and invoking the inference locally. It retrieves a job, downloads the input, calls the inference server to process the input, uploads the output and then returns the job result. 
...
The logic above illustrates how to explicitly call a service using either **IPv4 (127.0.0.1)** or **IPv6 (::1)**. For **localhost**, if it is mapped to both IPv4 and IPv6 in the OS **/etc/hosts** file, whether **localhost** resolves to IPv4 or IPv6 depends on the image and system configuration.
...
Let’s build the image and push it to [the Docker Hub](https://hub.docker.com/repository/docker/saladtechnologies/misc/tags). 
...
Now we can run some tests. In this scenario, you are building a batch-processing system using a group of GPU-powered container instances to handle thousands of jobs within a specified timeframe. Each instance retrieves its task inputs from a job queue, such as AWS SQS, Salad Job Queue, or Salad Kelpie.
...
```Code 
docker run --rm -it --gpus all -v ~/data:/app/data -e HOST="0.0.0.0" -e PORT="8888" docker.io/saladtechnologies/misc:test
```
...
<img src="/tutorials/images/docker_run/docker_run2.png" />

Let’s create a container group by providing the docker run command through the SaladCloud Portal:

<img src="/tutorials/images/docker_run/docker_run3.png" />

### Environment Variables

The -e flag in the docker run command is used to pass information to the container, which can be directly mapped to [the Environment Variables](https://docs.salad.com/products/sce/environment-variables) settings on SaladCloud. 
...
<img src="/tutorials/images/docker_run/docker_run5.png" />

### Image Source

Ensure the repository, image name and tag are all correct in [the Image Source](https://docs.salad.com/products/sce/container-registries). 
If the container image is from a private repository, you will need to provide both the username and access token. These credentials will be used by SaladCloud to pull the image. 

<img src="/tutorials/images/docker_run/docker_run4.png" />

After pulling the image from the designated repository, SaladCloud will store it in its own repository, from which the image will be delivered to the Salad nodes running your instances. When the container group is stopped, all Salad nodes will be released and the images on these nodes will be also removed.

If your image has changed since the container group was created, you will need to [update the image source](https://docs.salad.com/products/sce/managing-deployments#editing-your-container-group-deployment) to run the new version. This can be done whether the container group is stopped or running, prompting SaladCloud to pull the latest image.
...
Your container group consists of multiple replicas or instances, with each instance running on a separate Salad node.
...
Additionally, downloading your image and starting the instance on these Salad nodes may take some time, potentially ranging from a few minutes to longer, depending on the image size and the network conditions of the nodes. Some nodes may download the image faster than others, leading to earlier startup times. 
...
<img src="/tutorials/images/docker_run/docker_run6.png" />
...
The --gpus all option in the command allocates all available GPUs on the host to the container, enabling GPU support for applications running inside it. SaladCloud currently supports only one GPU per container. When creating a container group, you can select multiple GPU types at a priority level [(Batch/Low/Medium/High)](https://docs.salad.com/products/sce/priority-pricing), along with the required vCPU, memory and disk space. SaladCloud will then allocate the appropriate Salad nodes that match these selections to run the container group.

<img src="/tutorials/images/docker_run/docker_run7.png" />
...
If you are using the Salad Job Queue to manage the tasks for your container group, you can use Salad Job Queue Worker (an executable binary) to replace io_worker.py in the example. This worker is designed to handle input/output operations and interact with your inference server. For more information about Salad Job Queue, please refer to [this link](https://docs.salad.com/products/sce/job-queues). 
...
The -it flag allows you to interact with a container through a terminal. While SaladCloud does not offer this option during container group creation, you can still interact with running instances using [the interactive terminal](https://docs.salad.com/tutorials/howtousethesaladterminal) after the group has been created to perform tests or troubleshoot issues.
...
<img src="/tutorials/images/docker_run/docker_run8.png" />
...
```Code 
docker run --rm -it --gpus all -p 8000:8888 -e HOST="::" -e PORT="8888" --entrypoint 'sh' docker.io/saladtechnologies/misc:test  -c 'python inference_server.py'
```
...
<img src="/tutorials/images/docker_run/docker_run9.png" />
<img src="/tutorials/images/docker_run/docker_run10.png" />

Let’s create a container group by providing the docker run command through the SaladCloud Portal:

<img src="/tutorials/images/docker_run/docker_run11.png" />
...
<img src="/tutorials/images/docker_run/docker_run12.png" />

The container gateway is a load balancer deployed in front of instances within a container group to expose their services. 
**Instances running on SaladCloud must be configured to listen on both IPv4 and IPv6 (or IPv6 only) to ensure their services can be accessed through the container gateway.** 
This is why “::” is used to launch the Flask server in the docker run command.
...
<img src="/tutorials/images/docker_run/docker_run13.png" />
...
Instead, upload large files to cloud storage first, such as AWS S3, Cloudflare R2, or YouTube. Then, send requests to the instances through the container gateway, including only the file URL and metadata. After receiving requests, the instances direclty download the inputs (specified in the requests)  from cloud storage. This approach allows instances to optimize downloading throughput from cloud storage and reduces data transfer and copy. 
...
[Health probes](https://docs.salad.com/products/sce/health-probes) offer an automated mechanism to initiate specific actions based on the status of containers within a container group. **You can think of a probe as a piece of code running alongside the inference server within the same container, allowing it to access the service locally or the shared file system**.

[Readiness probes](https://docs.salad.com/products/sce/health-probes/readiness-probes) are essential when deploying a container gateway, as they ensure that requests are only forwarded to an instance when it is fully prepared—for example, when the model has been loaded and warmed up. 
...
You can also use the [exec protocol](https://docs.salad.com/products/sce/health-probes#exec-protocol) to create a custom script for the readiness probe. 

Here is an example: This exec probe executes a Python script inside the instance and checks the exit code. If accessing http://localhost:80/health returns a status code of 200, the Python script returns zero, indicating a successful probe. If not, the probe is considered failed.
...
<img src="/tutorials/images/docker_run/docker_run14.png" />

Let’s look at another example: This exec probe runs another Python script inside the instance and checks the exit code. If the response from accessing **http://[::1]:5000/health-check** contains **'READY'**, the Python script returns zero, indicating a successful probe. Otherwise, the probe is deemed failed. 

This probe is more advanced than the previous one, as it checks not only the HTTP status code, but also the semantics of the response. It also explicitly uses IPv6 (::1) to access the local service instead of **localhost**, which may resolve to either IPv4 (127.0.0.1) or IPv6 (::1) depending on the image and system configuration.
...
<img src="/tutorials/images/docker_run/docker_run15.png" />
...
For simplicity, configure your server to use either IPv6 or dual-stack only when the container gateway is enabled, and explicitly use IPv6 (::1) in the probes using the exec protocol. In all other scenarios, such as when using a job queue,  configure your server with IPv4 and use IPv4 (127.0.0.1) or localhost in the exec probes.


### Command

The --entrypoint ‘sh’ and  -c ‘python inference_server.py’ options in the command are converted to the Command deployment within the container group. This feature allows you to provide or override the original ENTRYPOINT and CMD instructions specified in the Dockerfile. 
**With this flexibility, you don't need to modify the Dockerfile, rebuild and push the image every time you change settings and code or run different applications.**

<img src="/tutorials/images/docker_run/docker_run16.png" />
...
<img src="/tutorials/images/docker_run/docker_run17.png" />
...
<img src="/tutorials/images/docker_run/docker_run18.png" />
...
- Deploying a container group with a few instances can greatly reduce waiting time and enhance the developer experience without incurring additional costs. 
...
- The probe and the inference server must be accurately aligned on both IP address (IPv4 or IPv6) and port. For simplicity, configure your server to use either IPv6 (or dual-stack) only when the container gateway is enabled.
...
- If you run multiple applications using the Docker Compose commands, you will determine their suitability for SaladCloud and then map the YAML file to deployment settings of multiple container groups on the platform.