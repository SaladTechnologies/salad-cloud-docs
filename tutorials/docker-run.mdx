...
[the quickstart guide](/products/quickstart) to set up your account, organization and project on SaladCloud. This
tutorial assumes you have a basic understanding of containers and images, Python, Docker commands, job queues, and load
balancers.
...
from a job queue, such as AWS SQS, Salad Job Queue, or Salad Kelpie.
...
[the Environment Variables](/products/sce/environment-variables) settings on SaladCloud.
...
Ensure the repository, image name and tag are all correct in [the Image Source](/products/sce/container-registries). If
the container image is from a private repository, you will need to provide both the username and access token. These
credentials will be used by SaladCloud to pull the image.
...
image will be delivered to the Salad nodes running your instances. When the container group is stopped, all Salad nodes
will be released and the images on these nodes will be also removed.

If your image has changed since the container group was created, you will need to
[update the image source](/products/sce/managing-deployments#editing-your-container-group-deployment) to run the new
version. This can be done whether the container group is stopped or running, prompting SaladCloud to pull the latest
image.
...
Additionally, downloading your image and starting the instance on these Salad nodes may take some time, potentially
...
group, you can select multiple GPU types at a priority level [(Batch/Low/Medium/High)](/products/sce/priority-pricing),
along with the required vCPU, memory and disk space. SaladCloud will then allocate the appropriate Salad nodes that
match these selections to run the container group.
...
If you are using the Salad Job Queue to manage the tasks for your container group, you can use Salad Job Queue Worker
(an executable binary) to replace io_worker.py in the example. This worker is designed to handle input/output operations
and interact with your inference server. For more information about Salad Job Queue, please refer to
[this link](/products/sce/job-queues).
...
[the interactive terminal](/tutorials/howtousethesaladterminal) after the group has been created to perform tests or
troubleshoot issues.
...
is completed. **The maximum waiting time must be 100 seconds or less and is configurable.** This configuration ensures
that an instance processes only one request at a time. You should carefully adjust these settings based on your
...
[Health probes](/products/sce/health-probes) offer an automated mechanism to initiate specific actions based on the
status of containers within a container group. **You can think of a probe as a piece of code running alongside the
inference server within the same container, allowing it to access the service locally or the shared file system**.

[Readiness probes](/products/sce/health-probes/readiness-probes) are essential when deploying a container gateway, as
they ensure that requests are only forwarded to an instance when it is fully prepared—for example, when the model has
been loaded and warmed up.
...
<img src="/tutorials/images/docker_run/docker_run19.png" />

You can also use the [exec protocol](/products/sce/health-probes#exec-protocol) to create a custom script for the
readiness probe.
