...
- **[Container Registries](/container-engine/container-registries)**: Specify the container image you want to run in the Container Group.
...
- **Additional Parameters**:
  - **[Startup Probe](/container-engine/health-probes/startup-probes)**: Set up a startup probe to check if the container is ready to accept traffic.
  - **[Liveness Probe](/container-engine/health-probes/liveness-probes)**: Configure a liveness probe to check if the container is healthy.
  - **[Container Gateway](/container-engine/networking)**: Choose between authentication or no authentication for external requests and port number for enabling networking inside of container.
  - **[External Logging Service](/container-engine/external-logging)**: Optionally configure an external logging service for container logs.
  - **[Environment Variables](/container-engine/environment-variables)**: Define environment variables to customize your container's environment.
...
Note: For detailed instructions on deploying containers, please refer to our "[Deploy Container Group](/container-engine/quickstart)" documentation.
