...
network, how SaladCloud works, unique traits of our distributed network, the choice of consumer GPUs over datacenter
...
- **What if a host tries to access my container?**
  ...
  - Our constant host intrusion detection tests look for operations like folder access, opening a shell, etc. If a host
    ...
    - This diagram explains the SaladCloud architecture to protect workloads from malicious software and suppliers from
      ...
      <img src="/container-engine/images/startingsalad1.png" />

- **What are some unique traits of Salad compared to other clouds?**
  - Since Salad is a computeshare network, our GPUs have longer cold start times than usual, and are subject to
    interruption.
...
- **What are Salad Endpoints/APIs?**
  - Salad has two API offerings today - a Transcription API and a Dreambooth API for stable diffusion fine-tuning. We
...
- **What should I be aware of before deploying a workload to Salad?**
  ...
  - If you're looking to implement networking on your instances within a container group by leveraging [Salad's container gateway](https://docs.salad.com/container-engine/networking), you'll need to [configure IPv6](https://docs.salad.com/container-engine/gateway/enabling-ipv6) within your container image.
...
- **What is the expected uptime on SaladCloud?**
  ...
  - Here is a real example over 24 hours for a production AI image generation workload with 100 requested nodes. As we
    ...
    <img src="/container-engine/images/startingsalad2.png" />

- **What is the right Salad product for my workload?**
  - Salad has four product lines: 

    - [Salad Container Engine (SCE)](https://docs.salad.com/container-engine/quickstart): a managed container engine built for deploying GPU-intensive containers at a massive scale.
    - [Salad Gateway Service (SGS)](https://docs.salad.com/gateway-service/introduction): a proxy service that routes requests from VPN operators or other customers who need distributed residential IP infrastructure to Salad nodes.
    - [Salad Transcription API](https://docs.salad.com/inference-endpoints/transcription/transcription): the lowest-priced transcription service in the market with unparalleled accuracy.
    - [Salad Dreambooth API](https://docs.salad.com/inference-endpoints/dreambooth/avatar-video-tutorial): a Stable Diffusion 1.5 and SDXL LoRA model training API with a full suite of tunable parameters.
