...
<Info>
  You can [learn more](https://explainshell.com/explain?cmd=curl+-s+-H+%22Metadata%3A+true%22+--noproxy+%22*%22+http%3A%2F%2F169.254.169.254%2Fv1%2Fstatus) about this command and its arguments on the awesome explainshell.com website.
</Info>
...
Other endpoints are provided to get an identity token for the running container instance and to reallocate the container instance to another Salad Node. Refer to the [IMDS API Reference](/container-engine/imds/api-reference) for more details.
...
response = client.metadata.get_container_status()
