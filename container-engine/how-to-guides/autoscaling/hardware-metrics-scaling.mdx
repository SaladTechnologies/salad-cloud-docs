...
monitoring GPU, CPU, and memory metrics and dynamically adjusting replica counts. Unlike queue-based autoscaling that
reacts to job volume, hardware utilization autoscaling responds to actual resource usage, making it ideal for
GPU-intensive workloads, real-time inference services, and applications with variable compute demands.
...
<Tip>
  **GPU Optimization**: Hardware utilization autoscaling is particularly effective for GPU workloads where you want to
  maintain optimal GPU utilization (typically 70-90%) while avoiding thermal throttling or memory exhaustion.
</Tip>
...
4. **Calling the SaladCloud API** to adjust replica counts
...
- **Cost optimization**: Right-size your deployment based on real utilization
...
Both approaches set deletion costs as utilization percentage (0-100) where higher values protect instances from
termination.

**IMDS Endpoints:**

- `GET /v1/deletion-cost` - Get current deletion cost
- `PUT /v1/deletion-cost` - Set deletion cost (0-100)
- `GET /v1/status` - Get instance readiness/startup status
- `GET /v1/token` - Get JWT for external authentication
...
def update_deletion_cost(client, utilization):
    ...
    deletion_cost = int(utilization)  # 0-100 based on utilization
...
<Tip>
  **Performance Best Practice**: Initialize the `SaladCloudImdsSdk` client once at the start of your application and
  reuse it throughout your code. This avoids the overhead of creating new connections and improves performance,
  especially in tight loops or frequent operations. Pass the client instance to functions that need it rather than
...
<Tip>
  **Error Handling**: The IMDS SDK automatically handles common error scenarios like network timeouts and temporary
  ...
  the need for manual retry logic.
</Tip>
...
<Tip>
  **Performance Best Practice**: Always place imports and client initializations (like `boto3.client()`,
  `initialize_datadog()`) at the module level, outside of functions. This avoids repeated initialization overhead and
  improves performance, especially in loops or frequently called functions.
</Tip>
...
def metrics_collection_loop():
    ...
    while True:
        try:
            ...
            deletion_cost = int(max_utilization)  # 0-100 based on utilization
            try:
    ...
    if os.environ.get('USE_CENTRAL_DELETION_COST_MANAGEMENT', 'false').lower() == 'true':
        ...
        for instance in instances:
            ...
            if 'aggregate' in m:
                ...
                deletion_cost = int(max_util)  # 0-100 based on utilization percentage
...
```bash
SALAD_API_KEY=your_api_key SALAD_ORG=your_organization SALAD_PROJECT=your_project
CONTAINER_GROUP_NAME=your_container_group GPU_SCALE_UP_THRESHOLD=85 GPU_SCALE_DOWN_THRESHOLD=50
CPU_SCALE_UP_THRESHOLD=80 CPU_SCALE_DOWN_THRESHOLD=40 MIN_REPLICAS=1 MAX_REPLICAS=10 SCALE_UP_INCREMENT=2
SCALE_DOWN_INCREMENT=1 SCALE_UP_COOLDOWN=300 SCALE_DOWN_COOLDOWN=600 METRICS_SOURCE=cloudwatch
...
Add comprehensive monitoring to your autoscaler:
...
def publish_scaling_metrics(metrics: List[Dict], scaling_action: str,
                          current_replicas: int, desired_replicas: int):
    """Publish custom metrics to CloudWatch"""
    # Calculate aggregate metrics
    avg_gpu_util = statistics.mean([
        m['aggregate'].get('gpu_utilization', 0) for m in metrics
    ]) if metrics else 0

    avg_cpu_util = statistics.mean([
        m['aggregate'].get('cpu_utilization', 0) for m in metrics
    ]) if metrics else 0

    max_gpu_temp = max([
        m['aggregate'].get('gpu_temperature', 0) for m in metrics
    ]) if metrics else 0

    # Publish metrics
    cloudwatch.put_metric_data(
        Namespace='SaladCloud/Autoscaling',
        MetricData=[
            {
            ...
            },
            {
                ...
                'Value': len([m for m in metrics if assess_instance_health(m)['healthy']]),
                'Unit': 'Count',
                'Dimensions': [
                    {'Name': 'ContainerGroup', 'Value': CONTAINER_GROUP_NAME}
                ]
            ...
            MetricData=[
                {
                    ...
                    'Dimensions': [
                        {'Name': 'ContainerGroup', 'Value': CONTAINER_GROUP_NAME},
                        {'Name': 'ScalingAction', 'Value': scaling_action}
                    ]
                }
            ]
        )
...
```python
def check_alert_conditions(metrics: List[Dict]) -> List[Dict]:
    ...
    if not metrics:
        alerts.append({
            'severity': 'warning',
            'category': 'monitoring',
            'message': 'No hardware metrics available for autoscaling decisions'
        })
        return alerts

    # Check for thermal issues
    hot_instances = [m for m in metrics
                    if m.get('aggregate', {}).get('gpu_temperature', 0) > 85]
    if hot_instances:
        alerts.append({
            'severity': 'critical',
            'category': 'thermal',
            'message': f'Critical GPU temperature detected on {len(hot_instances)} instances',
            'instances': [m['instance_id'] for m in hot_instances]
        })

    # Check for memory pressure
    memory_pressure_instances = [m for m in metrics
                               if m.get('aggregate', {}).get('memory_utilization', 0) > 95]
    if memory_pressure_instances:
        alerts.append({
            'severity': 'warning',
            'category': 'memory',
            'message': f'High memory pressure on {len(memory_pressure_instances)} instances',
            'instances': [m['instance_id'] for m in memory_pressure_instances]
        })

    # Check for unhealthy instances
    unhealthy_instances = [m for m in metrics
                         if not assess_instance_health(m)['healthy']]
    if len(unhealthy_instances) > len(metrics) * 0.3:  # More than 30% unhealthy
        alerts.append({
            'severity': 'critical',
            'category': 'health',
            'message': f'{len(unhealthy_instances)}/{len(metrics)} instances are unhealthy',
            'instances': [m['instance_id'] for m in unhealthy_instances]
        })

    return alerts

def send_alerts(alerts: List[Dict]):
    """Send alerts via SNS or other notification service"""
    if not alerts:
        return
    ...
    if not topic_arn:
        print("SNS_TOPIC_ARN not configured, skipping alert notifications")
        return
    ...
    for alert in alerts:
        try:
            ...
            if 'instances' in alert:
                message['affected_instances'] = alert['instances']

            sns.publish(
                TopicArn=topic_arn,
                Subject=f"SaladCloud Alert: {alert['category'].title()} - {alert['severity'].title()}",
                Message=json.dumps(message, indent=2)
            )

            print(f"Sent {alert['severity']} alert for {alert['category']}")

        except Exception as e:
            print(f"Failed to send alert: {e}")

def assess_instance_health(metric: Dict) -> Dict[str, Any]:
    """Assess the health of an instance based on its metrics"""
    health_status = {
        'healthy': True,
        'issues': []
    }

    if 'aggregate' not in metric:
    ...
    gpu_temp = agg.get('gpu_temperature', 0)
    if gpu_temp > 85:
        health_status['healthy'] = False
        health_status['issues'].append(f'GPU temperature critical: {gpu_temp}°C')
    elif gpu_temp > 80:
        health_status['issues'].append(f'GPU temperature high: {gpu_temp}°C')

    # Check memory usage
    memory_util = agg.get('memory_utilization', 0)
    if memory_util > 95:
        health_status['healthy'] = False
        health_status['issues'].append(f'Memory utilization critical: {memory_util}%')
    elif memory_util > 90:
        health_status['issues'].append(f'Memory utilization high: {memory_util}%')

    # Check if metrics are stale (older than 5 minutes)
    metric_age = datetime.utcnow().timestamp() - metric.get('timestamp', 0)
    if metric_age > 300:  # 5 minutes
        health_status['healthy'] = False
        health_status['issues'].append(f'Stale metrics: {metric_age:.0f}s old')
...
Create a script to validate your autoscaling behavior:
...
def generate_load(target_url: str, duration: int, thread_count: int):
    """Generate sustained load to test autoscaling"""
    def worker():
        ...
        while time.time() - start_time < duration:
            try:
                ...
                if request_count % 100 == 0:
                    print(f"Thread sent {request_count} requests")
    ...
    print(f"Starting load test with {thread_count} threads for {duration} seconds")
    threads = []
...
def monitor_metrics(container_group: str, duration: int):
    """Monitor scaling metrics during the test"""
    # This would connect to your monitoring system
    start_time = time.time()

    while time.time() - start_time < duration:
        # Query CloudWatch metrics
        print(f"Monitoring metrics for {container_group}...")
        time.sleep(30)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Test autoscaling behavior')
    parser.add_argument('--url', required=True, help='Target URL for load testing')
    parser.add_argument('--duration', type=int, default=300, help='Test duration in seconds')
    parser.add_argument('--threads', type=int, default=10, help='Number of concurrent threads')
    parser.add_argument('--container-group', required=True, help='Container group name to monitor')
    ...
    monitor_thread = threading.Thread(
        ...
        args=(args.container_group, args.duration)
    )
    monitor_thread.start()

    # Run load test
    generate_load(args.url, args.duration, args.threads)
...
- **Cost Optimization**: Scale down during low utilization to reduce costs
- **Performance Assurance**: Scale up proactively to maintain application responsiveness
- **Thermal Protection**: Prevent GPU overheating that could damage hardware
