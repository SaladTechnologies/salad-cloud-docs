...
- **Lower Compute Costs**: GPU + CPU + RAM combined cost a fraction of comparable EC2 GPU instances
  - RTX 4090 setup: \$0.330/hr at high priority vs AWS g5.xlarge: \$1.006/hr
- **Transparent Component Pricing**:
  - **GPU Containers:** GPU hourly rate
  - **CPU-only Containers:** \$0.005/vCPU/hour + \$0.001/GB RAM/hour
...
| **Lowest** | Non-urgent processing     | **~40-55% additional savings**                     | Variable     |
...
**SaladCloud RTX 4090 (24 GB):** GPU rates are all-inclusive, so the 8 vCPU and 32 GB RAM carry no additional charge.

- High Priority: **\$0.330/hour total** (67% less than g5.xlarge)
- Medium: **\$0.273/hour**
- Low: **\$0.217/hour**
- **Lowest: \$0.160/hour** (84% less than g5.xlarge and 94% less than P4d per GPU)

```python
# Configure container group with Lowest priority for maximum savings
# This gives you roughly 50% off the high-priority GPU rate
container_group = sdk.container_groups.create_container_group(
    organization_name="your-org",
    project_name="your-project",
    request_body={
