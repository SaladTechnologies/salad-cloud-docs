...
To follow best practices for processing large files, we recommend architecting applications using a pull model rather than a push model:

- Put the large files in Cloud Storage, such as AWS S3，Cloudflare R2 or YouTube. Select the appropriate service tier and lifecycle policy (Hot/Warm/Cold/Archive) based on their access patterns to optimize costs.
- Following the principle of least privilege, grant access to these files for your containers on SaladCloud.
- Send the jobs (file URL and metadata) to the containers via the Container Gateway or a job queue.
- A group of containers retrieve the jobs, download and process the files and provide the results.

With this architecture, the cost effectiveness, resilience and throughput of workload will be enhanced significantly.
