...
[health probes (Startup/Liveness/Readiness)](/container-engine/explanation/infrastructure-platform/health-probes) on SaladCloud to
malfunction.
...
[this guide](/container-engine/how-to-guides/real-time-inference/build-redis-queue).
...
- Use Salad nodes as a complement to
  [your local development environment](/container-engine/explanation/llm/overview#local-deployment-to-saladcloud) when
  you need access to specific GPUs. This allows you to monitor resource usage and test your code and dependencies in a
  diverse computing environment.

- Perform
  [the single node test](/container-engine/how-to-guides/transcription/youtube-transcription-pipeline#single-node-test)
