...
Our 2025 test measured the startup times for 100 container instances at high-priority across all consumer GPU types and
regions, using a 5.53 GB image. This metric tracks the number of instances that became operational since startup:
...
- **Proactive Reallocation**: During periods of high demand, SaladCloud may reassign resources from lower-priority
...
Due to variations in instance startup times and uptimes, it is not possible to consistently achieve the full requested
compute capacity on SaladCloud. When a node goes offline and a replacement is allocated, additional time is required to
download and decompress the image before the new instance becomes operational.

Results from the same 2025 test show the instance run-to-request ratio over the 7-day period:
...
Here is another instance run from the same test, showing performance fluctuations caused by temperature changes: as GPU
temperature rose, performance declined.
