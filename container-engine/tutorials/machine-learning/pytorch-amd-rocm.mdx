...
- A versioned `rocm/pytorch` image compatible with the AMD architecture you intend to use. The image must ship ROCm 7.1
  or newer; see [AMD GPUs and ROCm](/container-engine/reference/amd-gpus#image-requirements).
...
[What the Host Provides](/container-engine/reference/amd-gpus#what-the-host-provides).
...
## `rocminfo` Device Check

`rocminfo` is the canonical device-readiness check on SaladCloud: it enumerates the ROCr agents and fails with a
diagnosable error when it cannot. Include it in the image and use it to identify the GPU's `gfx` target:
...
SaladCloud injects a DXG-capable `amd-smi` from the host and puts it first on `PATH`, so `amd-smi` works in the
container. The copy bundled in your image does not: it checks `/proc/modules` for an `amdgpu` kernel driver that WSL2
does not expose. Do not run a suggested `modprobe` command inside SaladCloud. Do not use `rocm-smi` as an acceptance
check either; it exits 0 even when it has not talked to a GPU. Use `rocminfo` and the PyTorch operation as the device
checks. See [Checking That the GPU Works](/container-engine/reference/amd-gpus#checking-that-the-gpu-works).
...
building the image; `rocminfo` is the canonical readiness check and is worth carrying in production images. SaladCloud
manages the GPU runtime; do not try to replace it from the container.

### AMD SMI Reports That `amdgpu` Is Not Loaded

You are running the image's own AMD SMI rather than the host build SaladCloud injects. Do not run `sudo modprobe amdgpu`
inside the container. Confirm that `/opt/rocm-wsl/bin` is still first on `PATH`, or invoke `/opt/rocm-wsl/bin/amd-smi`
directly. See [Checking That the GPU Works](/container-engine/reference/amd-gpus#checking-that-the-gpu-works).
...
- Review [Choosing AMD GPU Classes](/container-engine/reference/amd-gpus#choosing-amd-gpu-classes) before increasing
  replicas or adding another AMD class.
