  ...
  <Tab title="Salad AI Gateway">
    ...
    **Recommended: save your connection with the setup wizard.** After installing the plugin, run:
    ...
    <Accordion title="Alternative: supply the key for a single terminal session">
      ...
      ```bash
      export SALAD_API_KEY="your-ai-gateway-api-key"
      ```

      The plugin also accepts `SALAD_CLOUD_API_KEY`, but `SALAD_API_KEY` takes precedence if both are set. Use the
      explicit provider/model command in Step 5. Exporting a variable does not save it for future terminal sessions.
