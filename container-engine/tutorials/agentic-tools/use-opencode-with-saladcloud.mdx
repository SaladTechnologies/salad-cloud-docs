...
<Tabs>
  <Tab title="Salad AI Gateway">
    Export your Salad AI Gateway API key before asking OpenCode to discover the provider:
    ...
    <Note>
      OpenCode only makes providers available when their required environment variables are set. If you run the models
      command before exporting `SALAD_CLOUD_API_KEY`, OpenCode reports `Provider not found: salad-cloud` even when its
      models cache is current.
    </Note>
    ...
    To make Salad AI Gateway the default, create or edit `~/.config/opencode/opencode.json`:
...
1. Confirm the API key is available in the same terminal session:

   ```bash
   test -n "$SALAD_CLOUD_API_KEY" && echo "API key is set"
   ```

2. Export the key before refreshing the models catalog.
...
To avoid re-exporting the key in every terminal session, add the appropriate variable to your shell profile:

<Tabs>
  <Tab title="Salad AI Gateway">
    ```bash
    echo 'export SALAD_CLOUD_API_KEY="your-ai-gateway-api-key"' >> ~/.bashrc
    source ~/.bashrc
    ```
