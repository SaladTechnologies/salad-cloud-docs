...
to connect to a self-hosted model on SaladCloud.

Aider pairs well with SaladCloud because:

- **Per-hour pricing** means no per-token cost during long coding sessions
- **OpenAI-compatible API** works natively - no plugins or proxies required
