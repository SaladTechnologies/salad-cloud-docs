...
Fooocus comes with a set of pretrained checkpoints, loras, and vae, in addition to a prompt expansion model.
By default in our docker image, these models download on start.
However, we're going to add them to the docker image directly so our container can become productive much faster after it enters the "running" state.
...
Navigate to [http://localhost:8888/docs](http://localhost:8888/docs) in your browser to see the API docs for Fooocus API.

3. Test a Text-to-Image request

See [the docs](http://localhost:8888/docs#/GenerateV1/text2img_generation_v1_generation_text_to_image_post) for more information on submitting a text-to-image request.
...
4. Decode the base64 encoded string into your image.
You can do this in a free browser tool such as https://codebeautify.org/base64-to-image-converter
        
![Untitled](container-engine/guides/stable-diffusion-guide-images/sunset-base64-3.png)

or using CLI tools like `jq` and `base64`.
...
```
    ...
    2. Deploy your image on Salad, using either the [Portal](https://portal.salad.com) or the [Salad Public API](https://docs.salad.com/api-reference/container_groups/create-a-container-group)
        ...
        ![Untitled](/container-engine/guides/stable-diffusion-guide-images/deploy-fooocus-1.png)
        ...
        ![Untitled](/container-engine/guides/stable-diffusion-guide-images/deploy-fooocus-2.png)
        ...
        ![Untitled](/container-engine/guides/stable-diffusion-guide-images/deploy-fooocus-3.png)
        ...
        ![Untitled](/container-engine/guides/stable-diffusion-guide-images/deploy-fooocus-4.png)
...
## Interact with Your Deployment
    1. Wait for the deployment to be ready.
        1. First, Salad pulls your container image into our own internal high-performance cache.
            ...
            ![Untitled](/container-engine/guides/stable-diffusion-guide-images/deploy-fooocus-5.png)
            
        2. Next, Salad locates eligible nodes for your workload, based on the configuration you provided.
            
            ![Untitled](/container-engine/guides/stable-diffusion-guide-images/deploy-fooocus-6.png)
            
        3. Next, Salad begins downloading the cached container image to the nodes that have been assigned to your workload.
            
            ![Untitled](/container-engine/guides/stable-diffusion-guide-images/deploy-fooocus-7.png)
        ...
        4. Eventually, you will see instances listed as “running”, with a green check in the “ready” column.
            ...
            ![Untitled](/container-engine/guides/stable-diffusion-guide-images/deploy-fooocus-8.png)
    ...
    2. Submit your prompt to the provided Access Domain Name. You will get back a json response within 10-20 seconds. See [above](#build-and-test-your-docker-image) for how to submit the request and process the response.
        ...
        ![Untitled](/container-engine/guides/stable-diffusion-guide-images/deploy-fooocus-9.png)