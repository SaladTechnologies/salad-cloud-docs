...
## Find a Docker Image
Find a docker image of Foocus. 
Here is one that we have verified works on Salad:
...
Note that we're using fooocus-api, and not the official Fooocus image.
This is because fooocus does not have a REST API, and we need to use a wrapper to make it work with Salad.
