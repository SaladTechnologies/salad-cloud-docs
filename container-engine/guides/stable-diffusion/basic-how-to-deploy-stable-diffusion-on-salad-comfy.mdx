...
Download any model files you plan to use. 
For our example, we’re going to use [Dreamshaper 8](https://civitai.com/models/4384/dreamshaper), available on [Civitai.com](http://Civitai.com) ([https://blog.salad.com/civitai-salad/](https://blog.salad.com/civitai-salad/))
...
# We can specify to just mount the stable diffusion 1.5 endpoints.
ENV WORKFLOW_MODELS=sd1.5
...
We've chosen to include the ComfyUI version (0.0.8) and the model name (dreamshaper8) in the tag.
    
```bash
docker build -t saladtechnologies/comfyui:0.0.8-dreamshaper8 .
```
...
2. (Recommended) Run the docker image locally to confirm it works as expected
    ...
    saladtechnologies/comfyui:0.0.8-dreamshaper8
    ```
    ...
    2. Click “Queue Prompt” to generate an image. Mine came out like this.
        ...
        ![Purple galaxy landscape inside a glass bottle on an outdoor table](/container-engine/images/stable-diffusion/comfy-image-1.png)
    ...
    4. Click the “Save (API Format)” button, and save it. You’ll get a file called “workflow_api.json” that contains everything ComfyUI needs to run that prompt again.
    ...
    5. Submit the prompt to the wrapper API on port 3000, using Postman or any http request tool of your choice.
        ...
        ![Submitting the request via Postman](/container-engine/images/stable-diffusion/comfy-submit-prompt-1.png)
        
        You should submit a POST request to [`http://localhost:3000/prompt`](http://localhost:3000/prompt) with a JSON request body like this, where the value of “prompt” is that workflow json we created previously.
    ...
    7. Decode the base64 encoded string into your image. You can do this in a free browser tool such as https://codebeautify.org/base64-to-image-converter
        ...
        ![Decoding the returned base64 image](/container-engine/images/stable-diffusion/comfy-decoded-image-1.png)
...
```bash
docker push saladtechnologies/comfyui:0.0.8-dreamshaper8
```
...
![Untitled](/container-engine/images/stable-diffusion/deploy-comfy-1.png)

![Untitled](/container-engine/images/stable-diffusion/deploy-comfy-2.png)
...
1. Wait for the deployment to be ready.
    ...
    4. Eventually, you will see instances listed as “running”, with a green check in the “ready” column.
        ...
        ![Untitled](/container-engine/images/stable-diffusion/deploy-comfy-8.png)
...
2. Submit your prompt to the provided Access Domain Name. You will get back a json response within a few seconds. See [above](#build-and-test-your-docker-image) for how to submit the request and process the response.
    ...
    ![Untitled](/container-engine/images/stable-diffusion/comfy-submit-prompt-2.png)