...
![Deploy Flux on Salad, generated in flux](/container-engine/images/stable-diffusion/deploy-flux-5.png)
...
## Find a Docker Image
Find a docker image of ComfyUI. 
Here's one we've verified works on Salad:

**ComfyUI**
- Git Repo: [https://github.com/ai-dock/comfyui](https://github.com/ai-dock/comfyui)
- Docker Image: [ghcr.io/ai-dock/comfyui:v2-cuda-12.1.1-base-22.04-v0.0.8](http://ghcr.io/ai-dock/comfyui:v2-cuda-12.1.1-base-22.04-v0.0.8)
...
We're going to use the Flux1-schnell model, at 8-bit precision.
You can [download it here](https://huggingface.co/Comfy-Org/flux1-schnell/blob/main/flux1-schnell-fp8.safetensors).
We're using the `schnell` version because it's the fastest, and it has a license allowing for commercial use.
We're using this convenient fp8 checkpoint because it runs well on consumer gpus, and allows for a simpler workflow in ComfyUI.

## Create a Dockerfile
1. Create a new file called `Dockerfile` and open it in your preferred text editor.
...
2. Save an API-Formatted workflow.json file in the same directory as your Dockerfile.
This file will be used to warm up the server before starting it.
You can generate this file by using the ComfyUI web interface to create a prompt, and then saving it in API format.
See [below](#build-and-test-your-docker-image) for more details.
...
# We also need to copy the comfyui-wrapper binary into the image, since ComfyUI
# is fully asyncronous by default, and has no convenient way to retrieve 
# generated images.
ADD https://github.com/SaladTechnologies/comfyui-wrapper/releases/download/1.2.0/comfyui-wrapper .
RUN chmod +x comfyui-wrapper

# OPTIONAL: Warmup the server by running a workflow before starting the server.
# The comfyui-wrapper supports a warmup mode, where it will run a provided workflow before starting the server.
...
# The comfyui-wrapper has some built in simple templated endpoints for common workflows.
# We can specify to just mount the flux endpoints.
ENV WORKFLOW_MODELS=flux

CMD ["./comfyui-wrapper"]
```

Note that we are including a simple wrapper binary to the image to make it easier to retrieve generated images.
ComfyUI accepts prompts into a queue, and then eventually saves images to the local filesystem.
This makes it difficult to use in a stateless environment like Salad.
This additional binary extends the ComfyUI `/prompt` API to allow either receiving the generated images in the response body, or having complete images submitted to a provided webhook url.

## Build and Test Your Docker Image
1. Build the docker image.
You should change the specified tag to suit your purpose.
    
```bash
docker build -t saladtechnologies/comfyui:0.0.8-flux1-schnell-fp8 .
```
...
2. (Recommended) Run the docker image locally to confirm it works as expected
    ...
    saladtechnologies/comfyui:0.0.8-flux1-schnell-fp8
    ```
    ...
    1. Go to [http://localhost:8188/](http://localhost:8188/) in your browser. You should see something like this: 
        ...
        ![Default Comfy UI Interface](/container-engine/images/stable-diffusion/comfy-default-view.png)
    ...
    ![Drag in the image to populate the workflow](/container-engine/images/stable-diffusion/flux_schnell_checkpoint_example.png)
    
    You should see something like the following:
    ![Populated workflow](/container-engine/images/stable-diffusion/comfy-default-view-flux.png)

    1. Click “Queue Prompt” to generate an image.
    The first image should be identical to the one we used to populate the workflow, because they use the same seed.
    If you generate another image, it should be different.
    Here's mine.
        
        ![A glass bottle with a galaxy and the planet earth inside, a glass containing white wine, a plate with mixed vegetables, and a plate with a piece of pie.](/container-engine/images/stable-diffusion/comfy-flux-image-1.png)
        
    1. Enable Dev Mode Options via the settings menu
        
        ![Click the gear to open the settings menu](/container-engine/images/stable-diffusion/comfy-settings-menu-1.png)
        
        ![Check the box labeled "Enable Dev mode Options"](/container-engine/images/stable-diffusion/comfy-settings-menu-2.png)
        
        You should see a new option in the menu, “Save (API Format)”:
        
        ![Notice the new button labeled "Save (API Format)"](/container-engine/images/stable-diffusion/comfy-settings-menu-3.png)
        
    1. Click the “Save (API Format)” button, and save it. You’ll get a file called “workflow_api.json” that contains everything ComfyUI needs to run that prompt again.
...
```
    ...
    5. Submit the prompt to the wrapper API on port 3000, using Postman or any http request tool of your choice.
        ...
        ![Submitting the request via Postman](/container-engine/images/stable-diffusion/comfy-flux-submit-prompt-1.png)
    ...
    7. Decode the base64 encoded string into your image. You can do this in a free browser tool such as https://codebeautify.org/base64-to-image-converter
        ...
        ![Decoding the returned base64 image](/container-engine/images/stable-diffusion/comfy-flux-decoded-image-1.png)
...
```bash
docker push saladtechnologies/comfyui:0.0.8-flux1-schnell-fp8
```
    
2. Deploy your image on Salad, using either the [Portal](https://portal.salad.com) or the [Salad Public API](https://docs.salad.com/api-reference/container_groups/create-a-container-group)

![Untitled](/container-engine/images/stable-diffusion/salad-create-container-group.png)

We’re going to name our container group something obvious, and fill in the configuration form.
Flux is a significantly larger model than previous image generation models, so we're going to want pretty robust hardware.

![Untitled](/container-engine/images/stable-diffusion/deploy-comfy-flux-fp8-1.png)

![Untitled](/container-engine/images/stable-diffusion/deploy-comfy-flux-fp8-2.png)

![Untitled](/container-engine/images/stable-diffusion/deploy-comfy-3.png)
...
![Untitled](/container-engine/images/stable-diffusion/deploy-comfy-4.png)
...
1. Wait for the deployment to be ready.
    1. First, Salad pulls your container image into our own internal high-performance cache.
        ![Untitled](/container-engine/images/stable-diffusion/deploy-comfy-flux-fp8-5.png)
    1. Next, Salad begins downloading the cached container image to the nodes that have been assigned to your workload.
        ![Untitled](/container-engine/images/stable-diffusion/deploy-comfy-flux-fp8-6.png)
    ...
    1. Eventually, you will see instances listed as “running”, with a green check in the “ready” column.
        ...
        ![Untitled](/container-engine/images/stable-diffusion/deploy-comfy-flux-fp8-8.png)
...
2. Submit your prompt to the provided Access Domain Name. You will get back a json response within a few seconds. See [above](#build-and-test-your-docker-image) for how to submit the request and process the response.
    ...
    ![Untitled](/container-engine/images/stable-diffusion/comfy-flux-submit-prompt-2.png)