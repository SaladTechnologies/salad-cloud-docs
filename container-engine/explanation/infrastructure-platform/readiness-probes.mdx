...
Readiness probes are only available if [Container Gateway](/container-engine/explanation/infrastructure-platform/networking)
or [Job Queues](/container-engine/explanation/job-processing/job-queues) are turned on. They run after the startup probe
