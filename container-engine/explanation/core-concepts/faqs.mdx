...
SaladCloud has one API offering today - a full-featured [Transcription API](/transcription/explanation/overview). We will be adding
...
- [Salad Container Engine (SCE)](/container-engine/tutorials/quickstart): a managed container engine built for
  deploying GPU-intensive containers at a massive scale.
- [Salad Gateway Service (SGS)](/gateway-service/explanation/overview): a proxy service that routes requests from VPN operators or
  other customers who need distributed residential IP infrastructure to SaladCloud nodes.
- [Salad Transcription API](/transcription/explanation/overview): the lowest-priced transcription service in
...
[external logging](/container-engine/explanation/core-concepts/external-logging). Axiom is our preferred external
logging service provider. Your logs are seamlessly transmitted to Axiom for troubleshooting.

SaladCloud users can also view their logs [directly in the portal](/container-engine/explanation/container-groups/container-logs).

SaladCloud users can [access a terminal](/container-engine/tutorials/development-tools/interactive-terminal) in a running container instance.
...
- Network performance will [vary from node to node](/container-engine/explanation/core-concepts/overview#performance-variability), due the
...
[SaladCloud's container gateway](/container-engine/explanation/networking-security/networking), you'll need to
...
- Similar to "spot" instances, SaladCloud using a
  [Priority Pricing](/container-engine/explanation/billing-pricing/priority-pricing) model, where you can
...
[SaladCloud Terminal](/container-engine/tutorials/development-tools/interactive-terminal) to execute commands on the container.
