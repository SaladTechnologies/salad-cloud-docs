...
<Tip>
  **Don't want to manage infrastructure?** [Salad AI Gateway](/ai-gateway/explanation/overview) gives you direct access
  to powerful open-source models through a single OpenAI-compatible endpoint - no container groups, no replicas, no cold
  starts. It's the fastest way to start using LLMs on SaladCloud.
</Tip>
