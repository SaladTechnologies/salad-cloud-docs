...
def resolve_redirect_chain(source: str, redirect_map: Dict[str, str], max_depth: int = 20) -> Tuple[str, List[str]]:
    ...
    while current in redirect_map and depth < max_depth:
        if current in visited:
            ...
            print(f"⚠️  Warning: Circular redirect detected in chain: {' -> '.join(chain)}")
            break
    ...
    if depth >= max_depth:
        print(f"⚠️  Warning: Max redirect depth ({max_depth}) reached for source: {source}")
...
def resolve_all_redirects(docs_json_path: str, dry_run: bool = False) -> Tuple[int, int, int]:
    ...
    for i, redirect in enumerate(redirects_array):
        ...
        final_destination, chain = resolve_redirect_chain(destination, redirect_map)
    ...
    if not dry_run and changes_made > 0:
        print(f"💾 Saving updated docs.json with {changes_made} resolved redirects...")
...
def validate_redirects(docs_json_path: str) -> bool:
    ...
    if chains_found == 0:
        ...
        print(f"❌ Found {chains_found} redirect chains that still need resolution")
        return False

def main():
    """Main function"""
    if len(sys.argv) < 2:
        print("Usage: python resolve-redirect-chains.py <docs.json> [--dry-run] [--validate]")
        print()
        print("Options:")
    ...
    try:
        total_redirects, chains_resolved, changes_made = resolve_all_redirects(docs_json_path, dry_run)
