...
Once the SGS server is configured, we send a signal to Salad Nodes in the target country telling them to connect to that
SGS server. Nodes are selected for eligibility based on a combination of their most recently-tested network speeds
