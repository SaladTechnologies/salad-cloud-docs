...
Install all of them into every agent on your machine with one command:
...
```bash Claude Code
claude mcp add --transport http salad-docs https://docs.salad.com/mcp
```
...
{/\* TODO (agent readiness plan, step 4): uncomment when the saladcloud CLI is published.

## Use the saladcloud CLI

The `saladcloud` CLI is built for agents and CI first: every command is non-interactive, `-o json` emits one JSON
document with every key always present, errors are a one-line JSON document on stderr, and exit codes are stable (2
usage, 3 auth, 4 not found, 5 rate limited, 6 invalid input, 7 network). Writes verify themselves and report on stderr
when the API applied something other than what was asked. Install it from <RELEASES URL>, then in any project:

```bash
saladcloud agent init
```

writes a short SaladCloud section into the project's `AGENTS.md` (or `CLAUDE.md` under Claude Code) so coding agents
working in that repository know to use the CLI and where the skills are. \*/}
...
assume the instance stays up, assume one replica is enough, forget IPv6, ship a 40 GB image. Until the dedicated skill
for this exists, point the agent at these pages before it writes anything:
