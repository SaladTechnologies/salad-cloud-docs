---
title: 'Use Cases: SSH, VS Code, JupyterLab, Private Networking and Meshed Communication'
description: 'Securely integrate unlimited GPU resources from SaladCloud into your applications'
---

## Initial Setup

The features demonstrated in this guide are available with a Tailscale personal account and are also fully supported in
enterprise accounts. To get started, follow [the quickstart guide](https://tailscale.com/kb/1017/install) to create your
Tailscale account and tailnet, then add your first devices (e.g., your developer machine for accessing Salad GPU nodes).
You can use either the interactive methods (Admin Console or CLI with the login process) to seamlessly join your devices
into the tailnet.

In the example below, two private and isolated devices are connected to the **salad_demo** tailnet and can securely
access each other. The **dev-vscode** device on macOS, representing a development laptop used for testing and debugging
code on Salad nodes. The **app-server** device on Linux, which simulates hosting applications (such as client
applications, databases, queues, proxies, or identity brokers) that securely interact with Salad nodes over the tailnet.
...
Tailscale uses [keys](https://tailscale.com/blog/tailscale-key-management) to manage devices across both the control
plane and data plane. In the control plane, keys are used for device authentication, authorization, and management. In
the data plane, keys facilitate secure, encrypted communication between devices, ensuring that data is transmitted
safely within the tailnet.
...
reauthentication is not completed within this period, the device's keys will expire, causing connections to and from the
device to stop working.

For devices that need to remain operational for extended periods, such as the **app-server**, you can disable key expiry
to avoid the need for reauthentication. For detailed instructions, refer to
...
which is used for secure communication within the tailnet. **However, not all devices with the tailscale0 interface can
access their own Tailscale IPs locally, as this behavior depends on the operating system, network configuration, and
firewall settings.**

```bash app-server
ubuntu@app-server:~$ ifconfig tailscale0
tailscale0: flags=4305<UP,POINTOPOINT,RUNNING,NOARP,MULTICAST>  mtu 1280
        inet 100.88.37.62  netmask 255.255.255.255  destination 100.88.37.62
        inet6 fe80::bbe0:c403:ae6d:2af  prefixlen 64  scopeid 0x20<link>
        inet6 fd7a:115c:a1e0::7901:253f  prefixlen 128  scopeid 0x0<global>
        unspec 00-00-00-00-00-00-00-00-00-00-00-00-00-00-00-00  txqueuelen 500  (UNSPEC)
        RX packets 9268  bytes 693497 (693.4 KB)
        RX errors 0  dropped 0  overruns 0  frame 0
        TX packets 5166  bytes 468215 (468.2 KB)
...
# Test the connectivity from app-server to dev-vscode.
...
<img src="/guides/tailscale-integration/images/ts-ssh.png" />
...
<img src="/guides/tailscale-integration/images/ssh-policy.png" />
...
<img src="/guides/tailscale-integration/images/proxy.png" />
...
- Salad nodes can communicate with each other using TCP and UDP. However, ping does not work due to the use of ICMP;
  instead, you can use Tailscale ping. The ping appears to work from devices in the default model with the tailscale0
  interface to Salad nodes, because ICMP packets are intercepted and responded by the Tailscale agent running on these
  nodes.
...
The Dockerfile creates a containerized environment by using a base image, then installing essential utilities and
required dependencies. It also copies the required Python code and startup script into the image. Upon running the image
on a Salad node, the following steps are executed:

- The Salad node is added to a tailnet using a Tailscale [auth key](https://tailscale.com/kb/1085/auth-keys) and its
  [SALAD_MACHINE_ID](/products/sce/container-groups/environment-variables).

- Start the Tailscale SSH server on the node.
...
<img src="/guides/tailscale-integration/images/auth-key.png" />

## Deploying on SaladCloud and Adding Salad Nodes to the Tailnet
...
Now we have 4 devices in the tailnet, with 2 on SaladCloud:
...
<img src="/guides/tailscale-integration/images/portal.png" />
...
<img src="/guides/tailscale-integration/images/vscode.png" />
...
<img src="/guides/tailscale-integration/images/jupyterlab.png" />
...
Without using the Tailscale proxy, Salad nodes can still access the internet publicly. This allows them to interact with
external services, receiving requests or jobs, and delivering results, while remaining securely connected to the
tailnet.
