...
In the example below, two devices are connected to the **salad_demo** tailnet: the **dev-vscode** device on macOS,
representing a development laptop used for testing and debugging code on Salad nodes, and the **app-server** device on
Linux, which simulates hosting applications (such as client applications, databases, queues, proxies, or identity
brokers) that securely interact with Salad nodes over the tailnet.
