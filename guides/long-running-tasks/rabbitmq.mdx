...
def do_the_actual_work(job: dict, checkpoint: dict) -> int | None:
    ...
    print(f"Max steps: {job['steps']}")
    print(f"Starting step: {checkpoint['step']}")
    while checkpoint['step'] < job['steps']:
        # Simulate work
        print(f"Working on job {job['job_id']}, step {checkpoint['step']}")
        time.sleep(30)
...
Finally, we need to connect to RabbitMQ and start consuming messages from the queue.
...
def upload_checkpoint(job_id: str, checkpoint: dict) -> None:
    ...
    print(f'Checkpoint uploaded for job {job_id}')
