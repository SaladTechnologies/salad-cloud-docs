...
For this guide, we will consider two example python workloads: one that lasts five hours, and one that lasts two weeks.
We will set up a job queue and related resources, a storage bucket, a checkpoint saving system, and a simple
...
to use mostly the same settings as before, but we're going to name this queue `my-job-queue-dlq.fifo`, and not assign it
anything beyond the default permissions. For this one, we will enable "Redrive allow policy", and allow our first queue
as the source queue.
...
```python
from clients import sqs, s3, sqs_queue_url, r2_bucket_name
```
...
def heartbeat_job(receipt_handle: str, stop_signal: threading.Event):
    '''
    Send a heartbeat to the SQS queue to keep the job alive

    Parameters:
    - receipt_handle: str, the receipt handle of the message
    - stop_signal: threading.Event, a signal to stop the heartbeat
    '''
...
Now, we need a function for "doing the work", which in our case is just slowly calculating a sum.

```python
def do_the_actual_work(job: dict, checkpoint: dict):
    '''
...
```python
def process_job(job: dict, receipt_handle: str):
    ...
    heartbeat_thread = threading.Thread(
        target=heartbeat_job, args=(receipt_handle, heartbeat_stop_signal))
