...
By using our integrated custom LLM prompts, you can extract the important quotes from your transcriptions, along with the timestamps. To use this, you just need to add the `custom_prompt` input parameter to your transcription request. 
...
while True:
  ...
  try:
      ...
      response = response.json()
      if response["output"]:
          print(response["output"]["llm_result"])
          break
      else:
          print(f'Current job status is {response["status"]}')
  except Exception as e:
      if response["status"] == "created" or response["status"] == "pending" or response["status"] == "started" or response["status"] == "running":
          print(f'Current job status is {response["status"]}')
