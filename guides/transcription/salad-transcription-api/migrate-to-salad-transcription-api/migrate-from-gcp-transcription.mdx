---
title: 'Migrating from GCP Transcription'
---

Migrating from GCP Transcription to Salad Transcription API will require you to replace your GCP Python functions with
Salad compatible ones. Whilst we do not offer an SDK, we do have example Python functions that you can use in
replacement.
...
that returns the data like GCP. With Salad Transcription API, you'll use two steps. Submit the job, and then requst the
results.

### GCP Transcription

Your existing Python code for GCP Transcription should look something like this:
...
In the response from submitting your job with the above step, you'll recieve the Job ID which is required to obtain the
