...
while True:
  ...
  try:
      ...
      if result["status"] == "created" or result["status"] == "started" or result["status"] == "running" or result["status"] == "failed":
          print(f'Current job status is {result["status"]}')
...
storage_url = storage_url  # Replace with the signed URL returned from the previous step
